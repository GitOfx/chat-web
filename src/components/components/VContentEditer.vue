<template>
    ...
    <div>
        <div class="header">
            <v-drop-list v-model="selectH" :menu-children="menuChildren" :head-style="headStyle">
            </v-drop-list>
            <v-drop-list v-model="selectFont" :menu-children="menuFontChildren" :head-style="headStyle">
            </v-drop-list>
            <span class="choiceItem" :class="{seleted:containCmd('bold')}"
                  style="font-weight: bold"
                  @click="clickCmd('bold')">B</span>
            <span class="choiceItem" :class="{seleted:containCmd('italic')}"
                  style="font-style: italic;font-family:Georgia, serif;"
                  @click="clickCmd('italic')">I</span>
            <span class="choiceItem" :class="{seleted:containCmd('strikeThrough')}"
                  style="text-decoration: line-through"
                  @click="clickCmd('strikeThrough')">S</span>
            <div class="link-item">
                <img src="@/assets/link.png" class="choiceItem" style="width: 20px" @click="showLink = !showLink"/>
                <div class="link-pop" v-if="showLink" @mouseleave="showLink = false">
                    <p>地址<input type="text" placeholder="输入地址" v-model="linkAdd"/></p>
                    <button @click="insertLink"> 确定</button>
                </div>
            </div>
            <div style="display: inline-block">
                <label>
                    <input type="file" name="file" accept="image/png,image/jpg" style="display: none"
                           @change="selectImg">
    ...
    export default {
        ...
        methods: {
            ...
            insertLink() {
                ...
                this.showLink = false;
                if (!!this.linkAdd) {
                    let linkHtml = `<a href='${this.linkAdd}'>${this.selectContent}</a>`
            ...
            clickCmd(cmd, arg) {
                ...
                let exeResult = document.execCommand(cmd, false, arg);
                // if (cmd == 'b') {
                //     exeResult = document.execCommand("bold", false, null);
                // } else if (cmd == 'i') {
                //     exeResult = document.execCommand("italic", false, null);
                // } else if (cmd == 's') {
                //     exeResult = document.execCommand("strikeThrough", false, null);
                // } else if (cmd == 'h1') {
                //     exeResult = document.execCommand("heading", false, "h1");
                // } else if (cmd == 'h2') {
                //     exeResult = document.execCommand("formatBlock", false, 'h2');
                // } else if (cmd == 'h3') {
                //     exeResult = document.execCommand("formatBlock", false, "h6");
                // }

                let index = this.exedCmd.indexOf(cmd)
                // console.log("resukt ", cmd, exeResult, index)
                if (index == -1) {
